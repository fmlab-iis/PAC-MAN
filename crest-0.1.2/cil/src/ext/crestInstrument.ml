...
let save_partial = ref false
let partial_bounds = ref 100
...
let isAssume f = hasAttribute "crest_assume" f.vattr
...
object (self)
  ...
  method vinst(i) =
    match i with
      Call (None, Lval (Var f, NoOffset), [arg], loc) when isAssume f ->
        ChangeTo [Call (None, Lval (Var f, NoOffset), [mkPredicate arg false], loc)]
    | _ -> DoChildren
...
class crestInstrumentVisitor f =
  ...
  let assumeFunc       = mkInstFunc "Assume" [] in
  ...
  let mkAssume ()          = mkInstCall assumeFunc [] in
...
object (self)
  ...
  method vinst(i) =
    match i with
      ...
      | Call (None, Lval (Var f, NoOffset), args, _) when isAssume f ->
        ...
        ChangeTo [mkAssume(); i ; mkClearStack ()]
   ...
   *)
  method vfunc(f) =
    if shouldSkipFunction f.svar || isAssume f.svar then
      SkipChildren
    else
...
let addCrestInitializer f =
  let partialArg = ("partial", intType, []) in
  let crestInitTy = TFun (voidType, Some [partialArg], false, []) in
  ...
  let globalInit = getGlobInit f in
    ...
    prependToBlock [Call (None, Lval (var crestInitFunc), [integer (if !save_partial then !partial_bounds else 0)], locUnknown)]
                   globalInit.sbody
...
(* Added by Ming-Hsien Tsai *)
let partial_feature : featureDescr =
  { fd_name = "SavePartial";
    fd_enabled = ref false;
    fd_description = "the export of the partial execution generated the program instrumented by CREST";
    fd_extraopt = [
      ("--partial-bounds",
       Arg.Int (fun i -> partial_bounds := i),
       "<bound> Set the bounds for partial traces. Default is " ^ string_of_int !partial_bounds ^ ".")
    ];
    fd_post_check = false;
    fd_doit = function (f : file) -> save_partial := true }
