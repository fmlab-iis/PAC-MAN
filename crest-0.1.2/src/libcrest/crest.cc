...
__CREST_BOOL enable_partial = 0;
int partial_bounds = 0;
...
void save() {
  ...
  /* Output the decision vector. */
  std::ofstream decout("decisions", std::ios::out);
  for (std::vector<bool>::iterator it = decisions.begin() ; it != decisions.end(); ++it)
    decout << *it;
  decout << std::endl;
  decout.close();
}

void __CrestInit(int partial) {
  partial_bounds = partial;
  enable_partial = partial_bounds > 0;
...
void __CrestBranch(__CREST_ID id, __CREST_BRANCH_ID bid, __CREST_BOOL b) {
  ...
  bool dec = static_cast<bool>(b);
  decisions.push_back(dec);
  SI->Branch(id, bid, dec);

  if (enable_partial && decisions.size() >= partial_bounds) {
    printf("CREST: Reached the bound on the number decisions.\n");
    save();
    exit(0);
  }
}
...
void __CrestAssume(__CREST_ID id) {
  SI->Assume(id);
}
...
void __CrestULong(unsigned long* x) {
  pre_symbolic = 0;
  *x = (unsigned int)SI->NewInput(types::U_LONG, (addr_t)x);
}

void __CrestULongLong(unsigned long long* x) {
  pre_symbolic = 0;
  *x = (unsigned int)SI->NewInput(types::U_LONG_LONG, (addr_t)x);
}
...
void __CrestLong(long* x) {
  pre_symbolic = 0;
  *x = (unsigned int)SI->NewInput(types::LONG, (addr_t)x);
}

void __CrestLongLong(long long* x) {
  pre_symbolic = 0;
  *x = (unsigned int)SI->NewInput(types::LONG_LONG, (addr_t)x);
}
