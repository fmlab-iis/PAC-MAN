...
#ifndef ERROR_STRING
#define ERROR_STRING "Reached __VERIFIER_error"
#endif

#define PARTIAL_EXT ".partial"
  ...
  : public binary_function<ScoredBranch, ScoredBranch, bool>
{
  ...
  /* a vector of decision vectors */
  static std::vector<string> decision_vectors;

  /* a map from a decision vector to past solutions to the branch constraints corresponding to the decision vector */
  static std::map<string, std::set<map<var_t, value_t> > > past_solns;

  /* a set of infeasible decision vectors */
  static std::set<string> infeasibles;

  /* A flag indicating if the partial mode is enabled. */
  int partial_mode = 0;

  static void SearchAtExit() {
    /* Output the decision vectors. */
    std::ofstream decout("decision_vectors", std::ios::out);
    for (std::vector<string>::iterator it = decision_vectors.begin() ; it != decision_vectors.end(); ++it)
      decout << *it << std::endl;
    decout.close();
  }
...
void Search::LaunchProgram(const vector<value_t>& inputs) {
  ...
  */
  ...
  if (partial_mode)
    fp = popen((program_ + PARTIAL_EXT).c_str(), "r");
  else
    fp = popen(program_.c_str(), "r");
  if (fp == NULL) {
  ...
  while (fgets(path, sizeof(path) - 1, fp) != NULL) {
    string line(path);
    if (line == ERROR_STRING || line == error_string) {
...
void Search::RunProgram(const vector<value_t>& inputs, SymbolicExecution* ex) {
  ...
  /* Delete the previous execution trace. */
  unlink("szd_execution");

  DMSG("=== Run Program ===\n")
  DMSG("Inputs:")
  DBG(for (vector<value_t>::const_iterator it = inputs.begin(); it != inputs.end(); ++it) fprintf(stderr, " %lld", *it);)
  DMSG("\n")
  ...
  if (in) {
    ...
    in.close();
  } else if (partial_mode == 0) {
    /* Go into partial mode and run the program again. */
    printf("CREST: Switch to partial mode.\n");
    partial_mode = 1;
    num_iters_--;
    RunProgram(inputs, ex);

    return;
  } else {
    printf("CREST: Failed to read the previous execution.\n");
    abort();
  }
  ...
  DMSG("Constraints:\n")
  DBG(for (size_t i = 0; i < ex->path().constraints().size(); ++i) { \
      const SymbolicPred* p = ex->path().constraints()[i];           \
      fprintf(stderr, "  %s\n", p->ToString().c_str()); \
    }                                                                   \
    )
  DMSG("Assumptions:\n")
  DBG(for (size_t i = 0; i < ex->path().assumptions().size(); ++i) { \
      const SymbolicPred* p = ex->path().assumptions()[i];           \
      fprintf(stderr, "  %s at %zu\n", p->ToString().c_str(), ex->path().assumptions_idx()[i]); \
    }                                                                   \
    )

  bool last_assume = ex->path().assumptions_idx().size() > 0 &&
    ex->path().assumptions_idx().back() == ex->path().constraints().size();

  /* Run programs with new inputs if the execution ends at an unsatisfied assumption. */
  while (last_assume) {
    vector<value_t> input;
    if (SolveAssumption(*ex, &input)) {
      DMSG("=== Run Program ===\n")
      DMSG("Inputs:")
      DBG(for (vector<value_t>::const_iterator it = input.begin(); it != input.end(); ++it) fprintf(stderr, " %lld", *it);)
      DMSG("\n")
      ...
      DBG(for (size_t i = 0; i < ex->path().assumptions().size(); ++i) { \
        ...
        DMSG("last index: %zu\n", ex->path().assumptions_idx().back());
      last_assume = ex->path().assumptions_idx().back() == ex->path().constraints().size();
    } else
      break;
  }
    
  /* Read the decision vector. */
  std::string decision;
  std::ifstream decin("decisions", std::ios::in);
  std::getline(decin, decision);
  decin.close();
  decision_vectors.push_back(decision);

  DMSG("Decision vector: %s\n", decision.c_str())
...
void Search::RandomInput(const map<var_t,type_t>& vars, vector<value_t>* input) {
  ...
  bool Search::SolveAssumption(const SymbolicExecution& ex, vector<value_t>* input) {
    DMSG("=== Solve Assumption ===\n")

    const vector<SymbolicPred*>& constraints = ex.path().constraints();
    const vector<SymbolicPred*>& assumptions = ex.path().assumptions();
    const vector<size_t>& assumptions_idx = ex.path().assumptions_idx();

    DMSG("Constraints:\n")
    DBG(for (size_t i = 0; i < constraints.size(); ++i) {          \
        fprintf(stderr, "  %s\n", constraints[i]->ToString().c_str());  \
      }                                                                 \
      )
    DMSG("Assumptions:\n")
    DBG(for (size_t i = 0; i < assumptions.size(); ++i) {          \
        fprintf(stderr, "  %s\n", assumptions[i]->ToString().c_str());  \
      }                                                                 \
      )
    ...
    /* 
     * Insert branch conditions and assumptions in a right order.
     * Note that the order will affect the results of the incremental solving.
     */
    vector<const SymbolicPred*> cs;
    size_t j = 0;
    for (; j < assumptions.size() && assumptions_idx[j] == 0; j++)
      cs.push_back(assumptions[j]);
    for (size_t i = 0; i < constraints.size(); i++) {
      for (; j < assumptions.size() && assumptions_idx[j] <= i; j++)
          cs.push_back(assumptions[j]);
    ...
    DMSG("Solve:\n")
    DBG(for (size_t i = 0; i < cs.size(); ++i) {                      \
        fprintf(stderr, "  %s\n", cs[i]->ToString().c_str());           \
      }                                                                 \
      )

    bool success = YicesSolver::IncrementalSolve(ex.inputs(), ex.vars(), cs, seen, &soln);

    if (success) {
      DMSG("Solution:\n")
      DBG(for (map<var_t, value_t>::iterator it = soln.begin(); it != soln.end(); ++it) { \
...
bool Search::SolveAtBranch(const SymbolicExecution& ex,
                           size_t branch_idx,
                           vector<value_t>* input) {
  DMSG("=== Solve At Branch ===\n")
  DMSG("Constraint index: %zu\n", branch_idx)

  const vector<SymbolicPred*>& constraints = ex.path().constraints();
  vector<const SymbolicPred*> assumptions;
  ex.path().assumptions(branch_idx, assumptions);
  const vector<size_t>& assumptions_idx = ex.path().assumptions_idx();
  ...
  /* Compute the next decision vector. */
  string prev_decvec = decision_vectors.back();
  DMSG("Previous decision vector: %s\n", prev_decvec.c_str())
    
  string next_decvec = prev_decvec.substr(0, branch_idx + 1);
  char last = next_decvec[next_decvec.length() - 1];
  next_decvec[next_decvec.length() - 1] = last == '0' ? '1' : '0';
  DMSG("Next decision vector: %s\n", next_decvec.c_str())

  if (infeasibles.find(next_decvec) != infeasibles.end())
    return false;

  /* Find past solutions. */
  std::map<string, std::set<map<var_t, value_t> > >::iterator it = past_solns.find(next_decvec);
  if (it == past_solns.end()) {
    std::set<std::map<var_t, value_t> > seen;
    past_solns[next_decvec] = seen;
  }
  it = past_solns.find(next_decvec);
  std::set<std::map<var_t, value_t> >& seen = it->second;
  ...
  /* It may be the case that there is only one solution. */
  if (!success && seen.size() > 0) {
    soln.clear();
    seen.clear();
    success = YicesSolver::IncrementalSolve(ex.inputs(), ex.vars(), cs, seen, &soln);
  }
  ...
  if (success) {
    ...
    *input = ex.inputs();
    ...
    infeasibles.insert(next_decvec);
  }
