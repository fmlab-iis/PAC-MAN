...
extern bool prevent_used_solutions;
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 4; i < argc; ++i) {
    ...
    if (arg.compare("-p") == 0)
      prevent_used_solutions = true;
    else
      option = atoi(arg.c_str());
  }
  ...
  if (search_type == "-random") {
    ...
    if (argc <= 4) {
      ...
      strategy = new crest::BoundedDepthFirstSearch(prog, num_iters, option);
    }
    ...
    if (argc <= 4) {
      ...
      strategy = new crest::UniformRandomSearch(prog, num_iters, option);
    }
  } else {
