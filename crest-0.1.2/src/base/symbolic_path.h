...
class SymbolicPath {
 public:
  ...
  void Assume(SymbolicPred* constraint);
  ...
  const vector<SymbolicPred*>& assumptions() const { return assumptions_; }
  const vector<size_t>& assumptions_idx() const { return assumptions_idx_; }
  void assumptions(size_t id, std::vector<const SymbolicPred*>& vec) const;
  
 private:
  vector<branch_id_t> branches_;
  vector<size_t> constraints_idx_;
  vector<SymbolicPred*> constraints_;
  /* a list of assumed conditions */
  vector<SymbolicPred*> assumptions_;
  /* a list of positions of assumptions in constraints_ */
  vector<size_t> assumptions_idx_;
};
